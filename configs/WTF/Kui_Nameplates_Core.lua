
KuiNameplatesCoreSaved = {
	["2162_PERSONAL_FRAME_SIZE_TRANSITION"] = true,
	["226_AURAS_TRANSITION"] = true,
	["226_CLASSPOWERS_Y"] = true,
	["216_HEALTH_TRANSITION"] = true,
	["226_TARGET_SIZE"] = true,
	["profiles"] = {
		["default"] = {
		},
		["青烟"] = {
			["nameonly_target"] = false,
			["level_text"] = true,
			["fade_avoid_mouseover"] = true,
			["name_vertical_offset"] = -22,
			["health_text_friend_dmg"] = 1,
			["health_text"] = true,
			["fade_avoid_casting_hostile"] = true,
			["fade_non_target_alpha"] = 0.3,
			["bar_animation"] = 2,
			["execute_enabled"] = false,
			["nameonly_friends"] = false,
			["bar_texture"] = "Smoother",
			["auras_offset"] = 2,
			["colour_hated"] = {
				1, -- [1]
				0.2862745098039216, -- [2]
				0.1450980392156863, -- [3]
			},
			["font_size_normal"] = 12,
			["target_glow_colour"] = {
				1, -- [1]
				0.9529411764705882, -- [2]
				0.8862745098039215, -- [3]
				1, -- [4]
			},
			["frame_width"] = 160,
			["frame_height_target"] = 22,
			["nameonly"] = false,
			["fade_avoid_nameonly"] = false,
			["auras_centre"] = false,
			["colour_neutral"] = {
				1, -- [1]
				0.8, -- [2]
				0, -- [3]
			},
			["frame_width_target"] = 180,
			["frame_height"] = 18,
			["bot_vertical_offset"] = -6,
			["frame_glow_size_target"] = 6,
			["frame_height_personal"] = 14,
			["frame_padding_x"] = 0,
			["nameonly_friendly_players"] = false,
			["font_face"] = "聊天",
			["fade_avoid_tracked"] = true,
			["fade_avoid_casting_interruptible"] = true,
			["frame_glow_size_shadow"] = 5,
			["font_style"] = 4,
			["fade_conditional_alpha"] = 0.1,
		},
	},
}
